...
`data` argument is the input data frame to be plotted `x_var` argument is the name of the variable to be plotted on the x-axis, `y_var` the name of the variable to be plotted on the y-axis, `x_label` is the label for the x-axis, `y_label` is the label for the y-axis,`plot_title` is the title for the plot, `plot_width` is the width of the plot and`plot_height` is the height of the plot.

For exmaple, we can use `horizontal_hist` function by providing `data = cannabis_and_nicotine`, `x_var = n`, `y_var = labels`, `x_label = "Amount of Participants"`, `y_label = "Labels"`, `plot_title = "Nicotine usage vs Cannabis usage"`, `plot_width = 10`, and `plot_height = 5`. This will create a horizontal histogram as it can be seen below.
...
`data` argument is the input data frame to be plotted, `x_var`argument is the name of the variable to be plotted on the x-axis, `y_var` argument is  the name of the variable to be plotted on the y-axis, `color_var` argument is the name of the variable to be used for color coding,  `x_label` arguemt is the label for the x-axis,
`y_label` argument is the label for the y-axis, `color_label` argument is the label for the color legend, `plot_title` arguement is the title for the plot, `plot_width` argument is the width of the plot, `plot_height` argument is the height of the plot.
...
scatterplot(example_data, Oscore, rate, Nicotine, "Openness", "Percentage of People Who Use Cannabis", "Use Nicotine", "Openness vs. Cannabis use", plot_width = 9, plot_height = 7)
```
